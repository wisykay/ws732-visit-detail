...
import {
  ...
  Settings, ChevronDown, User, LogOut, LayoutGrid, PieChart, ClipboardList, List, ArrowRight, ArrowLeft, PanelLeftClose,
    ...
    <div className="h-full flex flex-row bg-white shadow-sm overflow-hidden">
      ...
      <div className="w-1/2 flex flex-col h-full">
        <div className="px-6 py-4 bg-slate-50/50">
          <h2 className="text-xs font-bold text-slate-500 uppercase tracking-wider">Reglas de Validación</h2>
        </div>
          ...
          {BB2010_RULES.map((rule) => {
            ...
            return (
              <button
                ...
                className={`w-full flex items-center justify-between px-4 py-3 text-sm transition-all border-b border-slate-100 group ${isSelected
                  ? 'bg-blue-50/50 border-l-2 border-l-blue-400 border-b-slate-100'
                  : 'bg-white border-l-2 border-l-transparent hover:bg-slate-50/80'
                  }`}
              >
                <div className="flex items-center gap-3">
                  ...
                  <span className={`font-semibold text-left ${isSelected ? 'text-blue-700' : 'text-slate-700'}`}>{rule.label}</span>
                </div>
                <ChevronRight size={16} className={`${isSelected ? 'text-blue-500' : 'text-slate-300 group-hover:text-slate-400'}`} />
              </button>
            );
          })}
        </div>
      ...
      <div className="w-1/2 flex flex-col h-full bg-slate-50">
        <div className="px-6 py-4 bg-white flex justify-between items-center shrink-0">
          <h3 className="text-xs font-bold text-slate-500 uppercase tracking-wider">Evidencia Fotográfica</h3>
        </div>
    ...
    <div className="h-full flex flex-row bg-white overflow-hidden">
      ...
      <div className="w-1/2 flex flex-col h-full">
        <div className="px-6 py-4 border-b border-slate-200 bg-slate-50/50">
          <h2 className="text-xs font-bold text-slate-500 uppercase tracking-wider">Métricas SOS</h2>
        </div>
        <div className="flex-1 overflow-y-auto">
          {MOCK_SOS_CATEGORIES.map((cat, idx) => {
            ...
            return (
              <button
                ...
                className={`group relative w-full flex items-center justify-between px-4 py-3 text-sm transition-all overflow-hidden border-b border-slate-100 ${isSelected
                  ? 'bg-blue-50/50 border-l-2 border-l-blue-400 border-b-slate-100'
                  : 'bg-white border-l-2 border-l-transparent hover:bg-slate-50/80'
                  }`}
              >
                ...
                <div className="relative z-10 flex items-center justify-between w-full">
                  <span className={`font-semibold ${isSelected ? 'text-blue-700' : 'text-slate-700'}`}>{cat.label}</span>
                  <div className="flex items-center gap-3">
                    <span className={`text-lg font-semibold ${isSelected ? 'text-blue-700' : 'text-slate-700'}`}>{cat.percentage}%</span>
                    <ChevronRight size={16} className={`${isSelected ? 'text-blue-500' : 'text-slate-300 group-hover:text-slate-400'}`} />
                  </div>
                </div>
              </button>
    ...
    <div className="bg-white h-full flex flex-col w-full">
      ...
      <div className="px-6 py-3 flex items-center gap-4 bg-white overflow-x-auto no-scrollbar">
        ...
        <div className="flex items-center gap-2">
          <button
            ...
            className={`px-3 py-1.5 rounded-md text-xs transition-all border ${facingFilter === 'all'
              ? 'bg-blue-50 border-blue-200 text-blue-700 font-semibold'
              : 'bg-white border-slate-200/60 text-slate-600 font-medium hover:bg-slate-50 hover:border-slate-300'}`}
          >
            Todas ({counts.all})
          </button>
          <button
            onClick={() => setFacingFilter('with')}
            className={`px-3 py-1.5 rounded-md text-xs transition-all border ${facingFilter === 'with'
              ? 'bg-blue-50 border-blue-200 text-blue-700 font-semibold'
              : 'bg-white border-slate-200/60 text-slate-600 font-medium hover:bg-slate-50 hover:border-slate-300'}`}
          >
          ...
          <button
            ...
            className={`px-3 py-1.5 rounded-md text-xs transition-all border ${facingFilter === 'without'
              ? 'bg-blue-50 border-blue-200 text-blue-700 font-semibold'
              : 'bg-white border-slate-200/60 text-slate-600 font-medium hover:bg-slate-50 hover:border-slate-300'}`}
          >
      ...
      <div className="flex-1 overflow-y-auto">
        {paginatedProducts.map((prod: any, idx: number) => (
          <button key={idx} onClick={() => onProductClick(prod)} className={`w-full px-6 py-4 flex items-center justify-between hover:bg-slate-50 transition-all group text-left border-b border-slate-100 ${selectedProductId === (prod.id || prod.sku) ? 'bg-blue-50/50 border-l-2 border-l-blue-400' : ''}`}>
            <div className="flex items-center gap-3">
              ...
              <div className="min-w-0">
                ...
                <p className="text-[11px] font-medium text-slate-400">{prod.sku}</p>
              </div>
            </div>
            <div className="pl-2">
              <div className={`h-10 rounded-lg border flex items-center justify-center px-3 min-w-[48px] ${selectedProductId === (prod.id || prod.sku) ? 'bg-blue-100 border-blue-200 text-blue-700' : 'bg-slate-50 border-slate-200 text-slate-700'}`}>
                <span className="text-base font-semibold whitespace-nowrap">{showPrice ? (prod.price || '-- Bs.') : (prod.count || 0)}</span>
              </div>
            </div>
          </button>
        ))}
...
const App: React.FC = () => {
  ...
  const [ccuDesignMode, setCcuDesignMode] = useState<'scroll' | 'sidebar'>('sidebar');
  ...
  const renderSectionalDetail = () => {
    ...
    return (
      <div className="space-y-6 animate-in fade-in duration-500">
        <VisitHeader
          visit={visit}
          onBack={() => { setSelectedVisitId(null); setActiveTab('visits'); }}
        />
        ...
        <div className="space-y-6 animate-in fade-in slide-in-from-bottom-2 duration-300">
            ...
            <>
              <div className="grid grid-cols-1 md:grid-cols-3 gap-4 px-6 pt-6">
                ...
                <StatusCard type="score" label="SCORE" value={layoutFilter === 'failed' ? '1/4' : '3/4'} size="small">
                  <CircularProgress
                    ...
                    total={4}
                    size={60}
                    color={layoutFilter === 'failed' ? 'red' : 'green'}
                    showText={true}
                  />
                </StatusCard>
              ...
              <div className="mt-6">
                <div className="flex items-center justify-between mb-3 px-6">
                  <h3 className="text-xs font-bold text-slate-500 uppercase tracking-wider">Reglas de Validación</h3>
                  <div className="inline-flex bg-slate-100 rounded-md p-0.5 gap-0.5">
                    {(['all', 'CCU', 'CLIENTE'] as const).map(filter => (
                      <button
                        ...
                        className={`px-3 py-1.5 text-xs font-medium rounded transition-all ${ruleSourceFilter === filter
                  ...
                  {AA42937_LAYOUT_RULES.filter(rule => {
                    ...
                    return (
                      <button
                        ...
                        className={`w-full flex items-center justify-between px-4 py-3 rounded-none text-sm transition-all border-b border-slate-100 group ${isSelected
                    ...
                    >
                      <div className="flex items-center gap-3">
                        ...
                        <div className="flex flex-col items-start">
                          <span className={`font-semibold text-left ${isSelected ? 'text-blue-700' : 'text-slate-700'}`}>{item.label}</span>
                        </div>
                      </div>
                      <div className="flex items-center gap-3">
                        <span className="text-xs font-semibold bg-slate-100 text-slate-600 px-2 py-0.5 rounded-full">{item.count}</span>
                        <ChevronRight size={16} className={`${isSelected ? 'text-blue-500' : 'text-slate-300 group-hover:text-slate-400'}`} />
                      </div>
                    </button>
                  );
          ...
          {sectionId === 'etiquetas' && (
            <>
              ...
              <div className="mt-6">
                ...
                <div className="pt-4 border-t border-slate-100 mt-4 px-6">
                  <h3 className="text-xs font-bold text-slate-500 uppercase tracking-wider mb-2">Respuestas manuales</h3>
                  <div className="p-0">
                    <div className="flex justify-between items-start py-4 border-b border-slate-100 last:border-0 hover:bg-slate-50/50 transition-colors px-0">
                      <span className="text-slate-600 font-medium text-sm max-w-[70%]">Motivo de no ejecución</span>
                      <span className="text-blue-700 font-bold text-sm text-right">Precios en parte superior/inferior de la puerta</span>
                    </div>
                  </div>
                </div>
              </div>
          ...
          {sectionId === 'manuales' && (
            <div className="w-full pt-6 px-6">
              <h3 className="text-xs font-bold text-slate-500 uppercase tracking-wider mb-6">Respuestas manuales</h3>
              {MANUAL_QUESTIONS.map((item, idx) => (
                <div key={idx} className="flex justify-between items-start py-4 border-b border-slate-100 last:border-0 hover:bg-slate-50/50 transition-colors px-2">
                  <span className="text-slate-600 font-medium text-sm max-w-[70%]">{item.question}</span>
                  <span className="text-blue-700 font-bold text-sm text-right">{item.answer}</span>
                </div>
              ))}
            </div>
          )}
          ...
          {ccuDesignMode === 'sidebar' && (
            <div className="grid grid-cols-1 lg:grid-cols-12 gap-0 h-[calc(100vh-220px)]">
              ...
              <div className="lg:col-span-3 h-full overflow-hidden flex flex-col bg-white shadow-sm z-10">
                <div className="px-6 h-[72px] bg-slate-50/50 flex justify-between items-center">
                  <h3 className="text-xs font-bold text-slate-500 uppercase tracking-wider">Lista de encuestas</h3>
                  ...
                  {CCU_CATEGORIES.map(category => {
                    ...
                    return (
                      <button
                        ...
                        className={`w-full flex items-center justify-between px-3 py-2.5 rounded-md text-sm font-medium transition-all group border ${isActive
                          ? 'bg-blue-50 text-blue-700 border border-blue-200'
                          : 'text-slate-600 hover:bg-slate-50/80 hover:text-slate-900 border-transparent'
                          }`}
                      >
                        <div className="flex flex-col items-start gap-1 min-w-0 flex-1">
                          <span className={`truncate leading-tight w-full text-left text-sm ${isActive ? 'font-semibold text-slate-900' : 'font-semibold text-slate-700'}`}>{category.label}</span>
                          <span className="text-xs font-medium text-slate-500">{category.subtitle}</span>
                          <span className="mt-0.5 text-[10px] font-medium px-1.5 py-0.5 rounded border-0 bg-emerald-50 text-emerald-700">
                            {category.status}
                          </span>
                        </div>
                        <ChevronRight size={16} className={`flex-shrink-0 ${isActive ? 'text-blue-500' : 'text-slate-300 group-hover:text-slate-400'}`} />
                      </button>
                    );
                  })}
                </div>
              ...
              <div className="lg:col-span-9 h-full flex flex-col bg-white">
                ...
                <div className="px-6 h-[72px] flex items-center justify-between gap-3 shrink-0 bg-white">
                  <h2 className="text-lg font-bold text-slate-900 truncate">{CCU_CATEGORIES.find(c => c.id === ccuCategory)?.label}</h2>
                  <div className="inline-flex bg-slate-100 rounded-md p-0.5 gap-0.5">
                    {SECTIONS.map(section => (
                      <button
                        key={section.id}
                        onClick={() => setCcuSection(section.id as any)}
                        className={`px-3 py-1.5 text-xs font-medium rounded transition-all ${ccuSection === section.id ? 'bg-white text-slate-900 shadow-sm' : 'text-slate-600 hover:text-slate-900'}
                          }`}
                      >
                        {section.shortLabel}
                      </button>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-0 h-full">
          ...
          <div className="lg:col-span-3 flex flex-col h-[calc(100vh-250px)]">
            <div className="bg-white flex flex-col overflow-hidden h-full">
              <div className="px-6 h-[72px] bg-slate-50/50 flex justify-between items-center">
                <h3 className="text-xs font-bold text-slate-500 uppercase tracking-wider">{isDistribution ? 'Lista de encuestas' : detailSubTab === 'sos' ? 'Categorías' : 'Categoría'}</h3>
                ...
                {sidebarItems.slice((sidebarPage - 1) * itemsPerSidebarPage, sidebarPage * itemsPerSidebarPage).map(item => {
                  ...
                  return (
                    <button
                      ...
                      className={`w-full flex items-center justify-between px-3 py-2.5 rounded-md text-sm font-medium transition-all group ${isActive
                        ? 'bg-blue-50 text-blue-700 border border-blue-200'
                        ...
                        <div className="flex items-start justify-between gap-3 w-full">
                          <div className="flex-1 min-w-0 text-left">
                            ...
                            <div className="flex items-center gap-2 mt-1">
                              <span className={`text-[10px] font-medium px-1.5 py-0.5 rounded border-0 ${(item as any).status === 'DONE' ? 'bg-emerald-50 text-emerald-700' : 'bg-slate-100 text-slate-600'}`}>
                            ...
                            {isDistribution && (
                              <div className="flex items-center gap-2 mt-0.5">
                                {(item as any).status && (item as any).id !== 'precios' && (
                                  <span className={`text-[10px] font-medium px-1.5 py-0.5 rounded border-0 ${item.status === 'DONE' ? 'bg-emerald-50 text-emerald-700' : 'bg-slate-100 text-slate-600'}`}>
          ...
          <div className="lg:col-span-9 h-[calc(100vh-250px)]">
            ...
            <div className="h-full flex flex-col bg-white overflow-hidden">
              ...
              {isDistribution && (
                <div className="px-6 h-[72px] flex items-center justify-between gap-3 shrink-0">
                  <div className="flex items-center gap-3">
                  ...
                  {currentItem.id !== 'precios' && ( // Only show toggle for Distribution surveys (not Price Collections)
                    <div className="inline-flex bg-slate-100 rounded-md p-0.5 gap-0.5">
                      <button
                        ...
                        className={`px-3 py-1.5 text-xs font-medium rounded transition-all ${surveyViewMode === 'distribution' ? 'bg-white text-slate-900 shadow-sm' : 'text-slate-600 hover:text-slate-900'}`}
                      >
                        Distribución
                      </button>
                      <button
                        onClick={() => setSurveyViewMode('sos')}
                        className={`px-3 py-1.5 text-xs font-medium rounded transition-all ${surveyViewMode === 'sos' ? 'bg-white text-slate-900 shadow-sm' : 'text-slate-600 hover:text-slate-900'}`}
                      >
                        SOS
                      </button>
                      <button
                        onClick={() => setSurveyViewMode('rules')}
                        className={`px-3 py-1.5 text-xs font-medium rounded transition-all ${surveyViewMode === 'rules' ? 'bg-white text-slate-900 shadow-sm' : 'text-slate-600 hover:text-slate-900'}`}
                      >
                        Reglas
                      </button>
                    </div>
      ...
      <main className="flex-1 flex overflow-hidden relative bg-slate-50">
        <div className="flex-1 overflow-y-auto w-full">
          <div className="max-w-[1440px] mx-auto p-4 lg:p-8">
            {activeTab === 'visits' && !selectedVisitId && <VisitList onVisitSelect={handleVisitSelection} />}

            {(activeTab === 'overview' || (activeTab === 'visits' && selectedVisitId)) && (
              selectedVisitId ? (
